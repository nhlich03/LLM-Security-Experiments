...
| ✕ | **CAA** (Contrastive Activation Addition) | ACL 2024 | [nrimsky/CAA](https://github.com/nrimsky/CAA) | ✅ | ✅ (refusal 408/50) | **⚠️ LOẠI khỏi bảng chính (rà 28/07):** CAA là **steering ĐA DỤNG** — refusal chỉ 1 trong ~7 behavior (sycophancy, hallucination…), **paper KHÔNG nhắm thẳng jailbreak**. Cùng loại Arditi refusal-direction (§5.3) → related-work/baseline, không tính là defense jailbreak. |
| **1** 🆕 | **Jailbreak Antidote** | ICLR 2025 | ⚠️ chưa thấy repo chính thức → tự viết | tự viết (ngắn) | ✅ chỉ cần AdvBench/HarmBench + Alpaca (public) | ✅ **nhắm thẳng jailbreak** (tên bài). Training-free: **PCA** hidden-state benign vs harmful → "safety direction"; infer dịch biểu diễn chỉ ~5% chiều (sparse), 1 hệ số cân safety/utility |
...
| **1** 🆕 | **LED** (Layer-specific Editing) | Findings EMNLP 2024 | [ledllm/ledllm](https://github.com/ledllm/ledllm) | ✅ đủ (test Llama-2/Mistral) | ✅ AdvBench + safe responses (trong repo) | ✅ **nhắm THẲNG jailbreak** ("enhance resilience against jailbreak attacks"). **Weight-editing**: định vị "safety layer" tầng đầu → edit trọng số các layer đó realign câu an toàn. Intra nhẹ (edit vài layer, không full FT), 1 GPU. **KHÁC HỌ adversarial-training** → đa dạng survey. **Thay DINM** (agent rà 28/07). |
| ✕ | **DINM** | ACL 2024 (knowledge editing) | [zjunlp/EasyEdit](https://github.com/zjunlp/EasyEdit) | ✅ | ✅ SafeEdit (HF) | **⚠️ LOẠI (rà 28/07):** DINM từ paper **knowledge-editing / detox tổng quát**, KHÔNG phải paper chủ đích phòng thủ jailbreak (dù SafeEdit có vài jailbreak template). → thay bằng **LED** (bài trên). |
| 5 🆕 | **Safe Unlearning** | ⚠️ **chỉ arXiv 2024** (2407.02855) — venue yếu | [thu-coai/SafeUnlearning](https://github.com/thu-coai/SafeUnlearning) | ✅ đủ | ✅ trong repo (AdvBench + UltraFeedback) | Nhắm thẳng jailbreak ("...Defend Against Jailbreak Attacks", ASR 70%→<10% với ~100 mẫu). Machine-unlearning, **dễ nhất** — NHƯNG **chỉ arXiv** → dự phòng nếu nới tiêu chí venue. |
...
| 5 🆕 | **ReFAT** | **ICLR 2025** (arXiv 2409.20089) | ✕ không có repo public → tự viết | tự viết (thuật toán **ngắn**) | ✅ public (HarmBench/AdvBench + Alpaca) | Quan sát: **mọi** attack đều đi qua một cơ chế chung — ablate **refusal feature** trong residual stream. Vậy mô phỏng thẳng hiệu ứng cuối: xoá refusal feature với xác suất p qua các layer lúc forward, rồi SFT để model **vẫn từ chối**. Rẻ hơn R2D2 ~1700×, hơn CAT ~10×. ⚠️ **giá trị biên thấp** — cùng hướng refusal-direction với DeepRefusal + Circuit Breakers, làm cả 3 là trùng |

...
def parse_arguments():
    ...
    parser.add_argument('--input_path', type=str, default="datasets/harmfulqa/harmfulqs.json")
    parser.add_argument('--output_path', type=str, default="./outputs/harmfulqa")
...
def main(args):
    ...
    id=0
    for big_class in infra_data.keys():
        for sub_class in infra_data[big_class]:
            for instruction in tqdm(infra_data[big_class][sub_class]):
                ...
                if (id+1)%10==0:
                    with open(f"{args.output_path}/pos_output_harmfulqa.json", "w") as r:
                        json.dump(pos_all, r, indent=4)
                        
                    with open(f"{args.output_path}/neg_output_harmfulqa.json", "w") as r:
                        json.dump(neg_all, r, indent=4)
                        
                    with open(f"{args.output_path}/ours_output_harmfulqa.json", "w") as r:

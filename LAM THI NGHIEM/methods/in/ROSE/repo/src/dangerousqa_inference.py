...
def parse_arguments():
    ...
    parser.add_argument('--input_path', type=str, default="datasets/dangerousqa/dangerousqa.json")
    parser.add_argument('--output_path', type=str, default="./outputs/dangerousqa")
...
def main(args):
    ...
    infra_data=json.load(open(f"{args.input_path}"))
    ...
    tm_model = tm.TurboMind(args.model_path, eos_id=tokenizer.eos_token_id, tp=2)
    ...
    for id, instruction in tqdm(enumerate(infra_data)):
        ...
        if (id+1)%10==0:
            with open(f"{args.output_path}/pos_output_dangerousqa.json", "w") as r:
                json.dump(pos_all, r, indent=4)
                
            with open(f"{args.output_path}/neg_output_dangerousqa.json", "w") as r:
                json.dump(neg_all, r, indent=4)
                
            with open(f"{args.output_path}/ours_output_dangerousqa.json", "w") as r:
